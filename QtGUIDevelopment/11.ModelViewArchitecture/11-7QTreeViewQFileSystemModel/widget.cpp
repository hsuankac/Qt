    ...
    , ui(new Ui::Widget)
{
    ...
    dir_model->setFilter(QDir::NoDotAndDotDot | QDir::Files | QDir::Dirs);
    dir_model->setRootPath(path);

    /* The same "tree" model can be fed to both QListView and QTreeView. Even QTableView
     * The view will pick as much as it can handle from the model and displat the data
     */
    ui->treeView->setModel(dir_model);
    ui->listView->setModel(dir_model);

    // QStandardItemModel in tree mode
